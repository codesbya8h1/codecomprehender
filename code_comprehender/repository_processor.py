...
from code_comprehender.config import GITHUB_TOKEN, OUTPUT_DIR, get_performance_profile
...
from code_comprehender.repository_architecture_analyzer import (
...
class RepositoryProcessor:
    ...
    def __init__(
        ...
        self.max_concurrent_llm_requests = self.profile_settings[
            "max_concurrent_llm_requests"
        ]
        self.max_concurrent_viz_requests = self.profile_settings[
            "max_concurrent_viz_requests"
        ]
        ...
        self.logger.info(
            f"Initialized with '{performance_profile}' profile ({self.max_concurrent_files} concurrent files)"
        )
    ...
    def process_github_repository(
        ...
        with GitHandler(self.github_token) as git_handler:
            try:
                ...
                self.repository_info = git_handler.get_repository_info(
                ...
                self.logger.info(
                    f"Repository: {self.repository_info['name']} ({file_count} Java files)"
                )
                ...
                repo_output_dir = self._create_output_structure(
                    output_dir, self.repository_info["name"]
                )
                ...
                if use_parallel and len(java_files) > 1:
                    mode_desc = (
                        "documentation"
                        if documentation and not visualization
                        else "visualization"
                        if visualization and not documentation
                        else "analysis"
                    )
                    self.logger.info(
                        f"Processing {len(java_files)} files for {mode_desc} in parallel ({self.max_concurrent_files} concurrent)"
                    )
                    ...
                    try:
                        ...
                        successful_files = len(
                            [r for r in file_results if not r.get("errors")]
                        )
                ...
                if not use_parallel or len(java_files) == 1:
                    ...
                    self.logger.info(
                        f"Processing {len(java_files)} files for {mode_desc} sequentially"
                    )
                    # Use sequential processing (original method)
                    for i, java_file in enumerate(java_files):
                        log_progress(
                            f"Processing: {Path(java_file).name}",
                            i + 1,
                            len(java_files),
                        )
                        ...
                        try:
                            ...
                            successful_files += 1
                            log_progress(
                                f"Completed: {Path(java_file).name}",
                                i + 1,
                                len(java_files),
                            )

                        except Exception as e:
                            error_info = {"file": java_file, "error": str(e)}
                ...
                results["summary"] = {
                    ...
                    "architecture_analyzed_files": len(
                        self.architecture_analyzer.file_analyses
                    )
                    if visualization
                    else 0,
                }
                ...
                performance_metrics = {
                    ...
                    "files_per_minute": round((len(java_files) / total_time) * 60, 1)
                    if total_time > 0
                    else 0,
                    "avg_time_per_file": round(processing_time / len(java_files), 2)
                    if len(java_files) > 0
                    else 0,
                    ...
                    "total_relationships": len(
                        self.architecture_analyzer.file_relationships
                    )
                    if visualization
                    else 0,
                    "implicit_relationships_added": sum(
                        1
                        for rel in self.architecture_analyzer.file_relationships
                        if rel.get("type") == "implicit_jdbc"
                    )
                    if visualization
                    else 0,
                }
                ...
                self._log_performance_summary(
                    performance_metrics, successful_files, len(java_files)
                )

                log_success("Repository processing completed!")
                log_success(
                    f"Processed {successful_files}/{len(java_files)} files successfully"
                )
    ...
    def _auto_scale_concurrency(self, file_count: int):
        ...
        self.max_concurrent_files = min(
            int(self.max_concurrent_files * scale_factor), 20
        )

        # Scale LLM requests per file (but cap to avoid rate limits)
        self.max_concurrent_llm_requests = min(
            int(self.max_concurrent_llm_requests * scale_factor), 25
        )

        # Adjust rate limiting based on total load
        total_concurrent_requests = (
            self.max_concurrent_files * self.max_concurrent_llm_requests
        )
        ...
        if (
            self.max_concurrent_files != original_files
            or self.max_concurrent_llm_requests != original_llm
        ):
            self.logger.info(
                f"Auto-scaled to {self.max_concurrent_files} concurrent files for {file_count} files"
            )
        ...
        try:
            ...
            if visualization:
                ...
                architecture_analysis = (
                    self.architecture_analyzer.analyze_file_architecture(java_file)
                )

                # Save analysis data for the combined visualization
                file_name = Path(java_file).stem
                analysis_path = os.path.join(
                    analysis_output_dir, f"{file_name}_analysis.json"
                )
    ...
    def _create_combined_visualization(self, output_dir: str) -> str:
        ...
        viz_path = os.path.join(
            output_dir, "visualizations", "repository_architecture.html"
        )

        # Use the repository architecture analyzer to create the visualization
        result_path = self.architecture_analyzer.create_repository_visualization(
            viz_path, self.repository_info
        )
        ...
        summary_path = os.path.join(
            analysis_dir, "repository_architecture_summary.json"
        )
        ...
        if architecture_summary.get("entry_points") and architecture_summary.get(
            "exit_points"
        ):
            entry_count = len(architecture_summary["entry_points"])
            exit_count = len(architecture_summary["exit_points"])
            self.logger.info(
                f"Architecture: {entry_count} entry points, {exit_count} exit points"
            )
    ...
    def _analyze_cross_file_dependencies(
        self, all_nodes: Dict, file_packages: Dict
    ) -> List[Dict]:
            ...
            try:
                ...
                for import_stmt in imports:
                    ...
                    if (
                        ...
                        and imported_package in package_classes
                        and imported_class in package_classes[imported_package]
                    ):
                ...
                if package1 != package2:
                    ...
                    for node1_name, node1_info in nodes1:
                        for node2_name, node2_info in nodes2:
                            ...
                            if (
                                node2_info["type"] == "class"
                                and class2_name in node1_name
                                and node1_info["type"] in ["method", "class"]
                            ):
                                cross_calls.append(
                                    {
                                        "from": package1,
    ...
    def _add_repository_nodes(self, net, all_nodes: Dict, file_packages: Dict):
        ...
        method_nodes = [
            (name, info) for name, info in all_nodes.items() if info["type"] == "method"
        ]
        ...
        def method_priority(item):
            ...
            if method_name == "main":
                return 0
            if method_name == info.get("parent", "").split(".")[-1]:
    ...
    def _add_repository_edges(self, net, all_edges: List[Dict], all_nodes: Dict):
        ...
        for node_name, node_info in all_nodes.items():
            if (
                node_info["type"] == "class"
                and node_info["package"]
                and node_info["package"] in valid_nodes
        ...
        for node_name, node_info in all_nodes.items():
            if (
                ...
                and node_info["parent"]
                and node_info["parent"] in valid_nodes
                and node_name in valid_nodes
            ):
                net.add_edge(
                    node_info["parent"],
        ...
        for edge in all_edges:
            if (
                edge["type"]
                in ["package_dependency", "import_dependency", "usage_dependency"]
                and edge["from"] in valid_nodes
                and edge["to"] in valid_nodes
        ...
        for edge in all_edges:
            if (
                ...
                and edge["type"] in important_relationship_types
                and edge["from"] in valid_nodes
                and edge["to"] in valid_nodes
                and edge["from"] != edge["to"]
            ):
    ...
    def _process_files_parallel(
        ...
        progress_state = {"completed": 0, "total": len(java_files)}
        ...
        async def _process_all_files():
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    file_result = {
                        ...
                        "relative_path": str(
                            Path(java_files[i]).relative_to(repo_path)
                        ),
                        "documentation_path": None,
                        "visualization_path": None,
                        "analysis_path": None,
            ...
            self.logger.info(
                f"Completed {successful_count}/{len(java_files)} files in {processing_time:.1f}s"
            )
    ...
    async def _process_single_file_async(
        ...
        try:
            ...
            if documentation:
                ...
                documented_path = await self.async_documenter.process_file_async(
                    java_file, doc_file_dir
                )
                file_result["documentation_path"] = documented_path
        ...
        if progress_lock and progress_state:
            async with progress_lock:
                ...
                if total >= 20 and completed in [
                    total // 4,
                    total // 2,
                    (3 * total) // 4,
                ]:
                    self.logger.info(
                        f"🎯 {completed}/{total} files completed ({percentage:.1f}%)"
                    )
        else:
    ...
    def _log_performance_summary(
        self, metrics: Dict[str, Any], successful_files: int, total_files: int
    ):
        ...
        if metrics["total_relationships"] > 0:
            self.logger.info(
                f"Architecture: {metrics['total_relationships']} relationships found"
            )
        self.logger.info("=" * 60)
