...
class RepositoryArchitectureAnalyzer:
    ...
    def __init__(self, preset_name: str = None):
        ...
        self.logger.info(
            f"Repository Architecture Analyzer initialized with preset: {self.preset.name}"
        )
    ...
    def analyze_file_architecture(self, java_file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            max_tokens = (
                self.preset.max_chunk_size // 2
            )  # Use half for architecture analysis

            if total_tokens > max_tokens:
                # For large files, extract key architectural elements first
                code = self._extract_architectural_elements(code)
                self.logger.info(
                    f"Extracted architectural elements from large file ({total_tokens:,} tokens)"
                )
    ...
    def _extract_architectural_elements(self, code: str) -> str:
        ...
        for line in lines:
            ...
            if (
                ...
                or "extends" in stripped
                or "implements" in stripped
                or "new " in stripped
                and ("(" in stripped)
                or "main(" in stripped
            ):
    ...
    def _analyze_with_llm(self, code: str, file_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            messages = [
                ...
                HumanMessage(
                    content=f"Analyze this Java file for repository architecture:\n\nFile: {file_name}.java\n\n{code}"
                ),
            ]
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to parse LLM response for {file_name}: {e}"
                )
    ...
    def _create_fallback_analysis(self, file_name: str, code: str) -> Dict[str, Any]:
        ...
        return {
            "file_info": {
                ...
                if is_entry_point
                else ("exit_point" if is_exit_point else file_type),
    ...
    def _extract_file_relationships(self, file_name: str, analysis: Dict[str, Any]):
        ...
        self.logger.debug(
            f"Processing {len(dependencies)} dependencies for {file_name}"
        )
        ...
        for dep in dependencies:
            ...
            self.logger.debug(
                f"  Checking dependency: {file_name} -> {original_target} (normalized: {target})"
            )
            ...
            if not target_exists and not is_external_allowed:
                self.logger.debug("    SKIPPED: External dependency not allowed")
                continue
            ...
            self.logger.debug(
                f"    ADDED: {file_name} -> {target} ({relationship['type']})"
            )
    ...
    def _ensure_all_relationships_extracted(self):
        ...
        for file_name, analysis in self.file_analyses.items():
            ...
            for dep in dependencies:
                ...
                # Skip external dependencies that aren't in our analysis
                if (
                    target not in self.file_analyses
                    and not self._should_include_external_dependency(target)
                ):
                    self.logger.debug(
                        f"  SKIPPING external dependency: {file_name} -> {target}"
                    )
                    continue
                ...
                self.logger.debug(
                    f"Extracted: {file_name} -> {target} ({relationship['type']})"
                )
    ...
    def _add_implicit_relationships(self):
        ...
        for file_name, analysis in self.file_analyses.items():
            ...
            uses_driver_manager = any(
                "DriverManager" in dep.get("target_file", "")
                or "DriverManager" in dep.get("target_class", "")
                for dep in dependencies
            )
            uses_connection = any(
                "Connection" in dep.get("target_file", "")
                or "Connection" in dep.get("target_class", "")
                for dep in dependencies
            )
            ...
            if is_entry_point and (uses_driver_manager or uses_connection):
                ...
                for driver_name, driver_analysis in self.file_analyses.items():
                    ...
                    if "driver" in driver_name.lower() and (
                        "implements" in driver_purpose
                        or "driver interface" in driver_purpose
                    ):
                ...
                for driver_class in driver_classes:
                    ...
                    if not existing:
                        ...
                        implicit_added += 1
                        self.logger.debug(
                            f"Added implicit JDBC relationship: {file_name} -> {driver_class}"
                        )

        if implicit_added > 0:
            self.logger.info(
                f"Added {implicit_added} implicit architectural relationships"
            )

    def create_repository_visualization(
        self, output_path: str, repository_info: Dict[str, Any]
    ) -> str:
        ...
        for level, files in levels.items():
            ...
            for i, (file_name, analysis, file_type) in enumerate(files):
                ...
                if file_info.get("purpose"):
                    purpose = (
                        file_info["purpose"][:80] + "..."
                        if len(file_info["purpose"]) > 80
                        else file_info["purpose"]
                    )
                    tooltip_parts.append(
                        f"<div style='color: #34495e; margin: 8px 0; font-style: italic;'>{purpose}</div>"
                    )

                if dependencies:
                    tooltip_parts.append(
                        f"<div style='color: #e74c3c; font-weight: bold; margin-top: 8px;'>{len(dependencies)} Dependencies:</div>"
                    )
                    for dep in dependencies[:3]:  # Show up to 3 dependencies
                        target = dep.get(
                            "target_class", dep.get("target_file", "Unknown")
                        )
                        if target.endswith(".java"):
                            target = target[:-5]
                        rel_type = dep.get("relationship_type", "dependency")
                        tooltip_parts.append(
                            f"<div style='margin-left: 10px; color: #2980b9;'>• {target} ({rel_type})</div>"
                        )
                    if len(dependencies) > 3:
                        tooltip_parts.append(
                            f"<div style='margin-left: 10px; color: #95a5a6;'>... and {len(dependencies) - 3} more</div>"
                        )

                # Add interaction hints
                tooltip_parts.append(
                    "<div style='margin-top: 12px; padding-top: 8px; border-top: 1px solid #bdc3c7; color: #95a5a6; font-size: 11px;'>💡 Click to select • Drag to move • Hover to see connections</div>"
                )
    ...
    def _add_architecture_edges(self, net: Network):
        ...
        self.logger.info(
            f"Processing {len(self.file_relationships)} relationships for edges..."
        )
        ...
        for relationship in self.file_relationships:
            ...
            self.logger.debug(
                f"Checking relationship: {from_file} -> {to_file} ({rel_type})"
            )

            # Skip if source node doesn't exist
            if from_file not in self.file_analyses:
                self.logger.debug(
                    f"Skipping: source node {from_file} not found in analyses"
                )
                continue
            ...
            if to_file in self.file_analyses:
                ...
                from_type = (
                    self.file_analyses.get(from_file, {})
                    .get("file_info", {})
                    .get("file_type", "unknown")
                )
                to_type = (
                    self.file_analyses.get(to_file, {})
                    .get("file_info", {})
                    .get("file_type", "unknown")
                )
                ...
                self.logger.debug(
                    f"Skipping: target node {to_file} not found in analyses"
                )

        self.logger.info(f"Added {edges_added} edges to the visualization")

    def _save_architecture_json(
        self, html_output_path: str, repository_info: Dict[str, Any]
    ):
        ...
        architecture_data = {
            ...
            "files": {
                file_name: {
                    ...
                    "hierarchy_level": self._get_file_hierarchy_level(
                        analysis.get("file_info", {}).get("file_type", "service")
                    ),
                    "architectural_role": analysis.get("file_info", {}).get(
                        "file_type", "service"
                    ),
                    "is_entry_point": analysis.get("file_info", {}).get(
                        "is_entry_point", False
                    ),
                    "is_exit_point": analysis.get("file_info", {}).get(
                        "is_exit_point", False
                    ),
                }
                for file_name, analysis in self.file_analyses.items()
            ...
            "relationships": [
                {
                    ...
                    "from_level": self._get_file_hierarchy_level(
                        self.file_analyses.get(rel["from"], {})
                        .get("file_info", {})
                        .get("file_type", "service")
                    ),
                    "to_level": self._get_file_hierarchy_level(
                        self.file_analyses.get(rel["to"], {})
                        .get("file_info", {})
                        .get("file_type", "service")
                    ),
                }
            ...
            "file_types": {
                "entry_point": {
                    ...
                    "files": [
                        name
                        for name, analysis in self.file_analyses.items()
                        if analysis.get("file_info", {}).get("file_type")
                        == "entry_point"
                    ],
                },
                "controller": {
        ...
        try:
            ...
            self.logger.info(
                f"Repository architecture JSON analysis saved: {json_path}"
            )
    ...
    def _get_connectivity_metrics(self) -> Dict[str, Any]:
        ...
        most_dependencies = (
            max(out_degree.items(), key=lambda x: x[1]) if out_degree else ("None", 0)
        )
        most_dependents = (
            max(in_degree.items(), key=lambda x: x[1]) if in_degree else ("None", 0)
        )

        return {
            "total_connections": len(self.file_relationships),
            "average_connections_per_file": len(self.file_relationships)
            ...
            "isolated_files": [
                name
                for name in self.file_analyses.keys()
    ...
    def _identify_architecture_pattern(self) -> str:
        ...
        if (
            "controller" in file_types
            and "model" in file_types
            and "service" in file_types
        ):
            return "MVC (Model-View-Controller)"
        elif "service" in file_types and "data" in file_types:
            return "Service-Oriented Architecture"
        elif (
            len([f for f in file_types if f in ["utility", "service"]])
            > len(file_types) // 2
        ):
