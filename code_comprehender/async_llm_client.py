...
from code_comprehender.config import OPENAI_API_KEY, DEFAULT_MODEL, get_performance_profile
...
class AsyncLLMClient:
        ...
        async with self._semaphore:
            try:
                ...
                messages = [
                    ...
                    HumanMessage(content=f"Please add comprehensive documentation to this Java code:\n\n{code}")
                ]
                ...
                raise Exception(f"Failed to generate documentation for {chunk_name}: {e}")
    ...
    async def _document_chunk_with_progress(self, chunk: CodeChunk, current: int, total: int) -> str:
...
class AsyncLLMClientManager:
    ...
    @staticmethod
    def run_async_documentation(chunks: List[CodeChunk], performance_profile: str = "balanced", **kwargs) -> List[str]:
        ...
        """
        async def _document():
            async with AsyncLLMClientManager(performance_profile=performance_profile, **kwargs) as client:
