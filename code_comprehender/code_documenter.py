...
class CodeDocumenter:
    ...
    def _reconstruct_class_from_methods(self, method_chunks: List[tuple], reference_chunk: CodeChunk) -> str:
        ...
        class_name = first_method_name.split('.')[0] if '.' in first_method_name else "GeneratedClass"
    ...
    def _extract_method_from_documented_chunk(self, documented_chunk: str) -> str:
        ...
        for line in lines:
            ...
            if not in_method:
                ...
                if (('public ' in stripped or 'private ' in stripped or 'protected ' in stripped) and
    ...
    def _fix_common_syntax_issues(self, code: str) -> str:
        ...
        for line in lines:
            ...
            if imports_section and stripped.startswith('import '):
                ...
                continue
            elif stripped and not stripped.startswith('package ') and not stripped.startswith('//'):
                imports_section = False
            
            # Add missing Scanner import if Scanner is used but not imported
            if ('Scanner' in line and 'new Scanner' in line and 
                'import java.util.Scanner;' not in seen_imports):
        ...
        code_content = '\n'.join(fixed_lines)
        if 'Scanner' in code_content and 'import java.util.Scanner;' not in code_content:
            # Find the right place to insert the import
            final_lines = []
            inserted_import = False
            
            for line in fixed_lines:
                if not inserted_import and (line.strip().startswith('public class') or 
                                          (line.strip() and not line.strip().startswith('package') and 
    ...
    def _save_output(self, java_file_path: str, documented_code: str, output_dir: str) -> str:
