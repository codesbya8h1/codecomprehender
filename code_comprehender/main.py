...
from code_comprehender.logger import get_logger, log_success, log_warning, log_error, set_log_level
from code_comprehender.repository_processor import RepositoryProcessor

logger = get_logger()

@click.command()
@click.argument('java_file', type=click.Path(exists=True, dir_okay=False, path_type=Path), required=False) # This is just for testing
@click.option('--preset', '-p', default=None, help='LLM preset to use (default: gpt-4o-mini)')
@click.option('--output-dir', '-o', default=OUTPUT_DIR, help=f'Output directory (default: {OUTPUT_DIR})')
@click.option('--visualize', '-v', is_flag=True, help='Create architecture visualization')
@click.option('--github', '-g', help='GitHub repository URL to analyze')
@click.option('--docs-only', is_flag=True, help='Only create documentation (skip visualization)')
@click.option('--test-connection', is_flag=True, help='Test LLM connection and exit')
@click.option('--sequential', is_flag=True, help='Use sequential processing instead of parallel (slower)')
@click.option('--performance-profile', default='balanced', type=click.Choice(['conservative', 'balanced', 'aggressive']), 
              help='Performance profile: conservative (safe), balanced (default), aggressive (fast for large repos)')
def cli(java_file: Path, preset: str, output_dir: str, visualize: bool, github: str,
        docs_only: bool, test_connection: bool, sequential: bool, performance_profile: str):
    ...
    try:
        ...
        if github:
            ...
            logger.info(f"Repository: {results.get('repository_info', {}).get('name', 'Unknown')}")
            ...
            if results.get("errors"):
                log_warning(f"Some files failed to process. Check the processing report for details.")
        ...
        if not java_file:
            log_error("Please provide a Java file path or use --github for repository analysis")
            sys.exit(1)
