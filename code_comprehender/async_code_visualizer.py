...
from code_comprehender.java_parser import JavaParser, CodeChunk
...
class AsyncCodeVisualizer:
    ...
    async def analyze_code_architecture_async(self, java_file_path: str) -> Dict[str, Any]:
        ...
        if total_tokens <= max_tokens:
            ...
            self.logger.info(f"File exceeds token limit ({max_tokens:,}), chunking for analysis")
    ...
    async def _analyze_chunks_parallel(self, chunks: List[CodeChunk], 
                                     async_client: AsyncLLMClient) -> List[Dict[str, Any]]:
        ...
        for i, chunk in enumerate(chunks):
            task = self._analyze_chunk_with_progress(chunk, i + 1, len(chunks), async_client)
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                self.logger.warning(f"Failed to analyze chunk {chunks[i].name}: {result}")
    ...
    async def _analyze_chunk_with_progress(self, chunk: CodeChunk, current: int, total: int,
                                         async_client: AsyncLLMClient) -> Dict[str, Any]:
        ...
        """
        log_progress(f"Analyzing chunk: {chunk.name} ({chunk.token_count:,} tokens)", current, total)
        
        try:
            # Use async LLM analysis
            response_content = await self._analyze_chunk_with_llm_async(chunk.content, async_client)
    ...
    async def _analyze_chunk_with_llm_async(self, code: str, async_client: AsyncLLMClient) -> str:
        ...
        analysis_prompt = f"Analyze this code for architectural relationships:\n\n{code}"
        ...
        async with async_client._semaphore:
            try:
                ...
                response = await loop.run_in_executor(
                    None, 
                    lambda: async_client.llm.invoke(messages)
                )
    ...
    def create_visualization(self, output_path: str, file_name: str) -> str:
        ...
        """
        if not self.nodes:
            raise ValueError("No nodes found. Run analyze_code_architecture_async() first.")
        ...
        for node in self.nodes:
            if node.node_type == 'method' and node.parent and node.parent in valid_nodes:
        ...
        for relationship in self.relationships:
            ...
            if relationship.from_node not in valid_nodes or relationship.to_node not in valid_nodes:
                log_warning(f"Skipping edge {relationship.from_node} → {relationship.to_node}: missing node(s)")
                continue
        ...
        analysis_data = {
            "nodes": [{"name": node.name, "type": node.node_type, "parent": node.parent} 
                     for node in self.nodes],
            "relationships": [{"from": rel.from_node, "to": rel.to_node, "type": rel.relationship_type} 
    ...
    async def analyze_file_structure_async(self, java_file_path: str) -> Dict[str, Any]:
        ...
        analysis = await self.analyze_code_architecture_async(java_file_path)
    ...
    async def process_file_async(self, java_file_path: str, output_dir: str = "output") -> Tuple[str, str]:
    ...
    @staticmethod
    def process_files_parallel(file_paths: List[str], output_dir: str = "output", 
                             preset_name: str = None, performance_profile: str = "balanced") -> List[Tuple[str, str]]:
