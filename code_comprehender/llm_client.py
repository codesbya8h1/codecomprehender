...
from code_comprehender.config import OPENAI_API_KEY, DEFAULT_MODEL
...
class LLMClient:
    ...
    def document_chunks(self, chunks: List[CodeChunk]) -> List[str]:
        ...
        for i, chunk in enumerate(chunks):
            log_progress(f"Processing chunk: {chunk.name} ({chunk.token_count:,} tokens)", i + 1, len(chunks))
    ...
    def test_connection(self) -> bool:
        ...
        """
        try:
            test_message = SystemMessage(content="Respond with 'OK' if you can see this message.")
