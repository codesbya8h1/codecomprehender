...
class JavaParser:
            ...
            for class_decl in tree.types:
                if isinstance(class_decl, javalang.tree.ClassDeclaration):
                    class_chunks = self._extract_class_chunks_improved(
                        class_decl, lines, package_imports_header, max_tokens, package, imports
                    )
    ...
    def _extract_class_chunks_improved(self, class_decl: javalang.tree.ClassDeclaration, 
                                     lines: List[str], header: str, 
                                     max_tokens: int, package: str, 
    ...
    def _extract_method_chunks_improved(self, class_decl: javalang.tree.ClassDeclaration,
                                      lines: List[str], header: str,
                                      max_tokens: int, package: str, 
                                      imports: List[str]) -> List[CodeChunk]:
        ...
        for member in class_decl.body or []:
            if isinstance(member, javalang.tree.MethodDeclaration):
                ...
                method_end = self._find_method_end_line_improved(member, lines, method_start)
                
                method_lines = lines[method_start:method_end + 1]
                method_content = class_context + '\n\n    // Method implementation\n' + '\n'.join(['    ' + line for line in method_lines]) + '\n}'
    ...
    def _find_class_end_line_improved(self, class_decl: javalang.tree.ClassDeclaration, 
                                    lines: List[str]) -> int:
    ...
    def _find_method_end_line_improved(self, method_decl: javalang.tree.MethodDeclaration,
                                     lines: List[str], start_line: int) -> int:
    ...
    def _fallback_chunk_improved(self, code: str, max_tokens: int) -> List[CodeChunk]:
        ...
        for i, line in enumerate(lines):
            ...
            if (current_tokens + line_tokens + header_tokens > max_tokens and 
                current_chunk and is_logical_boundary):
        ...
        for chunk in chunks:
            if chunk.chunk_type in ['method', 'large_method']:
                if chunk.name not in seen_methods:
                    ...
                    self.logger.warning(f"Removing duplicate method chunk: {chunk.name}")
            else:
                deduplicated.append(chunk)
